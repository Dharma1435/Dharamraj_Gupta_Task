...
let sequelizeCon = new Sequelize("mysql://root@localhost/taskn");
...
app.post('/product/add', async(req,res)=>{
    ...
    if(findProduct){
        productData.slug = await generateUniqueslug(productData.slug,1)
    }
...
async function generateUniqueslug(baseslug,count){
    let newslug=`${baseslug}-${count}`
    let existingslug=await Product.findOne({where:{slug:newslug}})
    if(existingslug){
...
app.get('/product/:id', async(req,res)=>{
    let data = await Product.findOne({where:{id:req.params.id}}).catch((err)=>{
        return {error:err}
    })
    if(!data||(data&&data.error)){
...
app.delete('/product/delete/:id', async(req,res)=>{
    ...
    return res.send ({data: "product deleted Successfully"})
})
